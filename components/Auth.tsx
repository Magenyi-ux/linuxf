  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'SIGN_UP') {
        ...
        // Upgrade to Admin if email matches VITE_ADMIN_EMAIL
        if (formData.email === import.meta.env.VITE_ADMIN_EMAIL && userMatch.role !== 'ADMIN') {
          userMatch.role = 'ADMIN';
          userMatch.level = 99;
          const userIdx = users.findIndex((u: any) => u.email === formData.email);
          users[userIdx] = userMatch;
          localStorage.setItem('waExamPrep_users', JSON.stringify(users));
        }
