...
export const PracticeSession: React.FC<PracticeSessionProps> = ({
  ...
  const getOptionStyle = (idx: number) => {
    ...
    if (!isAnswered) {
      return (
        baseStyle + "border-gray-100 hover:border-brand-200 hover:bg-brand-50"
      );
    }
        ...
        {currentQuestion.options.map((option, idx) => (
          ...
          >
            <div className="flex items-center gap-4 z-10 relative">
              <span
                className={`w-8 h-8 flex items-center justify-center rounded-lg text-sm font-bold flex-shrink-0 border ${
                  isAnswered &&
                  mode === "STUDY" &&
