      ...
      {parts.map((part, i) => {
        ...
        if (math) {
          try {
            ...
            // Sanitize KaTeX output for extra safety (Defense in Depth)
          ...
          if (part.includes('<') && part.includes('>')) {
            // CRITICAL SECURITY: Sanitize raw HTML segments to prevent XSS
            const sanitizedPart = DOMPurify.sanitize(part);
            return <span key={i} dangerouslySetInnerHTML={{ __html: sanitizedPart }} />;
          }
